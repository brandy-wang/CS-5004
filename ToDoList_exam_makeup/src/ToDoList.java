...
public class ToDoList {
  ...
  void sortByImportance() {
    ...
    // Assuming ToDoItem has a method getImportance() which returns a Comparable (like Integer or Enum)
    Collections.sort(myToDoList, (item1, item2) -> item2.getImportance().compareTo(item1.getImportance()));
  }
  ...
  void sortByUrgency() {
    // STUDENT TO IMPLEMENT THIS METHOD
    // Assuming ToDoItem has a method getUrgency() which returns a Comparable (like Integer or Enum)
    Collections.sort(myToDoList, (item1, item2) -> Integer.compare(item2.getUrgency(),(item1.getUrgency())));
  }
